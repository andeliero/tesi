...
Questa sezione descrive il funzionamento del sistema realizzato durante lo stage.
In fase di startup il sistema non possiede nessun dato, e quindi non è in grado di fornire raccomandazioni. \\
Un modo per verificare lo stato operativo del modulo è attraverso una chiama a:\\
\begin{center}
GET    /tres/ready
\end{center}
...
Viene indicato con un booleano l'operatività del modulo. L'operatività dipende prima di tutto se è stato generato albero. La sua generazione è programmata ogni 24 ore e necessita di almeno 100 behavior per essere eseguita.\\



Per la procedura di inserimento dei comportamenti si usa la chiamata:
\begin{center}
POST    /tres/behavior
\end{center}
...
\end{lstlisting}
Un behavior non può contenere più interazioni con lo stesso Widget Tag e un Item deve contenere almeno un Tag.\\
Una volta generato l'albero è possibile richiedere la raccomandazione.\\
La prima chiamata da effettuare è:
