...
\item \textbf{WidgetTag:} questa entità rappresenta un elemento generico all'interno di un sito web, serve per individuare l'oggetto su cui l'utente svolge una azione. %widget tag deriva da tag perchè permesso da orientDb
...
\end{itemize}
%Da un \emph{behavior} si dirama l'associazione verso un \emph{Item} attraverso lo spigolo \emph{Result} ed esprime che il comportamento dell'utente ha come risultato un \emph{Item}. Un \emph{Item} è descritto con una serie di \emph{Tag}, quindi un vertice di tipo \emph{Item} è associato a molteplici \emph{Tag} tramite uno spigolo \emph{HoldsTag}. Un \emph{Behavior} descrive una serie di interazioni su dei \emph{WidgetTag}, quindi un vertice di tipo \emph{Behavior} è associato a molteplici \emph{WidgetTag} tramite uno spigolo \emph{Interaction}. \emph{Interaction} contiene la tipologia svolta su di un determinato \emph{WidgetTag}.
...
%Mentre per il testing di integrazione delle componenti dei controller ho seguito un approccio top-dow, mi sono servito di alcuni mock che mi hanno permesso di simulare le componenti da cui erano dipendenti.
...
\end{figure}
\newpage
In fase di startup il sistema non possiede nessun dato, e quindi non è in grado di fornire raccomandazioni.
Il client deve dunque verificare lo stato di operatività del sistema attraverso una chiamata HTTP a:\\
\begin{center}
GET    /tres/ready
\end{center}
...
\end{lstlisting}
La risposta HTTP ritorna un oggetto Json contentente un booleano che indica l'operatività del modulo. L'operatività dipende prima di tutto se sono stati inseriti abbastanza comportamenti utente, quindi se non è operativo si continua con la procedura di inserimento dei comportamenti tramite la chiamata:
\begin{center}
POST    /tres/behavior
\end{center}
L'oggetto Json da inviare tramite questa chiamata deve contenere un Json simile al seguente.
\begin{lstlisting}[language=json,firstnumber=1]
{
  "item":{
    "tags":[
...
\end{lstlisting}
Un comportamento si registra solamente quando le azioni dell'utente hanno prodotto un risultato all'interno del sito (come ad esempio un acquisto nel caso di un \emph{e-commerce}). Una volta generato l'albero, Tres è in grado di fornire raccomandazioni agli utenti e quindi si il Client può iniziare la procedura di per richiedere una raccomandazione. Il client innanzitutto deve richiedere a Tres con quale argomento interrogare l'utente mediante la chiamata HTTP a: 
\begin{center}
POST    /tres/answer
\end{center}
\newpage
La chiamata deve comprendere un oggetto Json che contiene le interazioni che l'utente web ha già svolto.
\begin{lstlisting}[language=json,firstnumber=1]
{
  "interactions":[
    {
...
\end{lstlisting}
Questa chiamata fornisce come risposta il prossimo WidgetTag da utilizzare per interrogare l'utente del sito web come il seguente oggetto Json.
\begin{lstlisting}[language=json,firstnumber=1]
{
  name: "Income"
}
\end{lstlisting}
L'utente va ad interagire con il WidgetTag con cui lo interroghiamo, così il client aggiunge l'interazione alle interazioni precedentemente salvate. Il client deve continuare a reiterare questa chiamata aggiungendo le interazioni finché Tres fornisce dei WidgetTag per interrogare l'utente. Se Tres non fornisce altri WidgetTag significa che la classificazione del comportamento è terminata ed è possibile per il client procedere con la richiesta della raccomandazione. La chiamata HTTP da effettuare è la seguente.
\begin{center}
POST    /tres/recommendation
\end{center}
Nella chiamata dev'essere inserito il Json contenente le interazioni dell'utente. La risposta di Tres comprende un oggetto Json dove vengono indicate le percentuali dei prodotti/oggetti/post da raccomandare all'utente.
\begin{lstlisting}[language=json,firstnumber=1]
{
  "items":[
    {
...
\end{lstlisting}
Quando il comportamento dell'utente ha prodotto un risultato il client inserisce il suo comportamento in Tres e termina la procedura.
%**************************************************************
